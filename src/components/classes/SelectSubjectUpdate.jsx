...
function SelectSubjectUpdate({ ind, subjectTeacherData }) {
  const {
    getAllSubjects,
    subjects,
    setSubjects,
  ...
  return (
    <div className="md:flex md:items-center mb-6 w-full">
      <div>
        ...
        >
          <option disabled>Fach...</option>

          {subjects.map((subject, index) => {
            if (subjectTeacherData.subject !== undefined) {
              if (subjectTeacherData.subject._id === subject._id) {
                return (
                  <option key={index} defaultValue={subject._id} selected>
