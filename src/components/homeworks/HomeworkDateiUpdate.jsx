...
export default function HomeworkDateiUpdate({ homeworkToUpdate }) {
  ...
  const [showHomeworkToUpdate, setShowHomeworkToUpdate] = useState(true);
  ...
  function getFileUploaded() {
    if (homeworkToUpdate.fileName.split(".")[1] === "pdf") {
      setPdfFile(homeworkToUpdate.link);
    } else {
      setPicture(homeworkToUpdate.link);
    }
  }

  const homeworkFileReplace = (e) => {
    setHomeworkUploaded(false);
    setShowHomeworkToUpdate(false);
